  ...
  if (scriptUrl && scriptUrl !== "your_google_script_url") {
    try {
      await fetch(scriptUrl, {
        ...
        body: JSON.stringify({ bookerName: booker.name, bookerEmail: booker.email, bookerWhatsapp: booker.whatsapp || "", hostName: host.name, hostEmail: host.email, hostWhatsapp: host.whatsapp || "", note: note || "" }),
      });
