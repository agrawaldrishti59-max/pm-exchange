...
export default function MemberPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Check if user already has active session with this member
      if (myData && m) {
  ...
  if (booked) return (
    <div style={{ padding: 32, textAlign: "center", maxWidth: 400, margin: "0 auto" }}>
      ...
      <h2>Request sent!</h2>
      <p style={{ color: "#666", lineHeight: 1.6, marginTop: 8 }}><strong>{member.name}</strong> has been notified. Once they accept, both of you will get a Google Meet link.</p>
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      <div style={{ background: "#fff", borderBottom: "1px solid #eee", padding: "48px 16px 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
          {member.avatar_url ? (
            <img src={member.avatar_url} alt={member.name} style={{ width: 60, height: 60, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} />
          ) : (
      ...
      <div style={{ flex: 1, padding: 16, overflowY: "auto" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #eee", borderRadius: 12, padding: 14, marginBottom: 12 }}>
          ...
          <a href={member.linkedin_url?.startsWith("http") ? member.linkedin_url : `https://${member.linkedin_url}`}
            target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, color: "#185FA5", wordBreak: "break-all", display: "flex", alignItems: "center", gap: 6 }}>
            <span>🔗</span> {member.linkedin_url}
          </a>
        </div>
          ...
          <div style={{ background: "#fff", border: "1px solid #eee", borderRadius: 12, padding: 14 }}>
            ...
            {me?.status !== "approved" && (
              <div style={{ background: "#FAEEDA", borderRadius: 8, padding: 10, marginBottom: 12 }}>
                <p style={{ fontSize: 13, color: "#854F0B", margin: 0 }}>⏳ Your profile is pending approval. You can browse but can't book yet.</p>
              </div>
            )}
            ...
            {!hasActiveSession && me?.status === "approved" && (
              <>
                <p style={{ fontSize: 13, color: "#666", lineHeight: 1.6, marginBottom: 12 }}>Select a slot above, then send your request. Costs <strong>1 credit</strong>. {member.name?.split(" ")[0]} will get notified and can accept — then you both get a Meet link.</p>
                ...
                <button style={{ width: "100%", padding: 13, background: !selectedSlot || (me?.credits ?? 0) < 1 ? "#ccc" : "#111", color: "#fff", border: "none", borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: !selectedSlot || (me?.credits ?? 0) < 1 ? "not-allowed" : "pointer" }}
                  ...
                  {booking ? "Sending request…" : !selectedSlot ? "Select a slot first" : "Send request · 1 credit"}
                </button>
              </>
            )}
          </div>
